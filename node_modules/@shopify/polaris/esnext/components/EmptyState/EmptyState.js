...
export function EmptyState({ children, heading, image, largeImage, imageContained, centeredLayout = false, action, secondaryAction, footerContent, }) {
    const withinContentContainer = useContext(WithinContentContext);
    const { newDesignLanguage = false } = useFeatures();
    const newLayout = centeredLayout || newDesignLanguage;
    const className = classNames(styles.EmptyState, newLayout && styles.centeredLayout, imageContained && styles.imageContained, withinContentContainer ? styles.withinContentContainer : styles.withinPage);
